...
//   術語（命盤/流年/紫微斗数…）沿用漢字（日本新字体）。之後加韓語比照新增 KO 表。
const JA = {
  '待付款': 'お支払い待ち',
...
// 四語挑選：繁中／简中／English／日本語（每次呼叫都讀當前 uiLang）
//   ja：第 4 參數（僅含動態插值的樣板字串使用）優先 → JA[繁中原文] → 缺譯時警告並退回英文。
export const pick = (tw, cn, en, ja) => {
  const L = getLang();
  if (L === 'ja') {
...
export const fmtDate = (iso) => {
  if (!iso) return '';
  try { const L = getLang(); return new Date(iso).toLocaleDateString(L === 'en' ? 'en-US' : (L === 'zh-CN' ? 'zh-CN' : (L === 'ja' ? 'ja-JP' : 'zh-TW')), { year:'numeric', month:'short', day:'numeric' }); }
  catch (_) { return iso; }
};
